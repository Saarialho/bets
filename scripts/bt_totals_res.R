...
oof_predictions <- preds %>%
  ...
  select(game_id, model_id, league, fo2.5:fu2.5, prob_under:prob_over) %>%
  pivot_longer(fo2.5:fu2.5, names_to = 'close', values_to = 'target') %>%
  mutate(pred = case_when(close == 'fo2.5' ~ prob_over,
                          TRUE ~ prob_under),
         side = factor(case_when(close == 'fo2.5' ~ '1',
                                 TRUE ~ '2'))) %>%
  mutate(league = factor(league)) %>%
  mutate(model_id = factor(model_id)) %>%
  select(-close, -prob_under, -prob_over) %>%
...
meta_model_preds %>%
  ...
  #taalla pitaa olla X!
  mutate(side_dummy = glue::glue('side_X{side}')) %>%
